// ============================================================
// TalaSaAI â€” All Mock Data (Thesis Prototype)
// ============================================================
const MOCK = {
  students: [
  ...
  questions: {
    ...
    p4:[
      { q:"Who wrote The Little Prince?",                               opts:["Victor Hugo","Antoine de Saint-ExupÃ©ry","Jules Verne","Albert Camus"], ans:1 },
...
const LEGACY_TEXT_PATTERN = /(?:Ãƒ.|Ã‚.|Ã¢.|Ã°Å¸|Ã¢â‚¬|Ã¢â‚¬â„¢|Ã¢â‚¬Å“|Ã¢â‚¬\u009d|Ã¢â‚¬Â¢|Ã¢â€ |Ã¢â€°|Ã¢Å“|Ã¢â€š)/;
...
function getCachedTalaSaAIUser() {
  try {
...
function normalizeAvatarSrc(value) {
  ...
  const basePath = getTalaSaAIBasePath();
...
function syncShellUserFooter(sidebar, currentPageName) {
  ...
  const cachedUser = getCachedTalaSaAIUser();
...
function initTalaSaAIThemeSync() {
  ...
  function syncThemeState() {
    ...
    applyChartTheme();
    window.dispatchEvent(new CustomEvent("TalaSaAI:themechange", {
      detail: {
  ...
  window.getTalaSaAIThemePreference = function() {
    return getStoredThemePreference() || document.documentElement.dataset.appearance || (colorQuery.matches ? "dark" : "light");
  };

  window.setTalaSaAIThemePreference = function(value) {
    storeThemePreference(value);
    syncThemeState();
  };

  window.clearTalaSaAIThemePreference = function() {
...
function initTalaSaAIShell() {
  ...
  if (banner) banner.textContent = "TalaSaAI System";
  ...
  themeButtons.forEach(function(button) {
    button.addEventListener("click", function() {
      if (typeof window.setTalaSaAIThemePreference === "function") {
        window.setTalaSaAIThemePreference(button.dataset.themeValue);
      }
    });
  });

  window.addEventListener("TalaSaAI:themechange", syncThemeButtons);
  ...
  const collapseKey = "TalaSaAI.sidebar.collapsed";
